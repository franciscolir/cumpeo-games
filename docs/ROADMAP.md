...
| 5a | 2026-09-25 | **Fase 5 (dashboard/flujo)**: formulario de circuito con N juegos (agregar/quitar + validaciones: nombre, 2 equipos `#RRGGBB`, ≥1 juego, sin duplicados) y botón **Comenzar** en la lista (solo `LISTO` → `crearPartida` con retry ×3 → `#/partidas/:id`). e2e circuitos reescrito + 2 nuevos → 8/8 (LocalAdapter). 2662 unit / 86 archivos. Commit código `3a93cb0`. Deuda **#140** (boot e2e contra Supabase requiere migración `0016`). |
---
