...
| 129 | Historia Enredada fase `VOTANDO` no puede usar el descriptor `input` del contrato: el binding del shell envía `{ valor: e.target.value }` (shell-partida.js:1471) pero el handler `asignar-puntos-historia` lee `payload.puntos || 0` (1241) → asignaría 0 puntos; un botón descriptor tampoco sirve (payload estático). Decisión operador 8.5b.1: `VOTANDO` devuelve `[]` → fallback legacy (input + botón con `{puntos}`), D3 queda incumplida en esta fase. Habilitar D3 exige modificar el shell (aceptar `{ valor }` o enriquecer el binding de `input`), requiere aprobación. | Paso 8.5b.1 (detectado) | Media |
| 130 | Descriptor `selector` del shell envía `{ valor }` (shell-partida.js:1467) pero `cambiar-turno-manual-memoria` lee `payload.equipo` (shell-partida.js:452): con selector, `MemoriaGameDefinition.cambiarTurno(..., undefined)` alternaría el equipo en vez de fijar el elegido (y `valor` es string). Resuelto en 8.5b.2 (M1-A) con 2 botones fantasma payload `{ equipo: 1|2 }` (paridad con el legacy). Revisar si conviene unificar el contrato `selector` (payload uniforme `{ valor }`) o que el handler acepte ambos. | Paso 8.5b.2 (detectado) | Media |
| 131 | Pictionary no migrado a `accionesConductor` (M2-A): devuelve `[]` en todas las fases → fallback total al panel legacy. El bonus requiere payload compuesto `{ equipo, puntos }` (shell-partida.js:1166-1167) que el contrato actual no expresa (el binding `input` envía `{ valor }` — shell:1471 — y el payload de botón es estático) y el bonus legacy está en todas las fases, así que migrar parcialmente el panel lo rompería. Resolver en 8.5c extendiendo el contrato (descriptor `input` con acción compuesta) o migrando el bonus a un Extra. | Paso 8.5b.2 (detectado) | Media |
| 132 | Rosco migrado parcialmente (M3-A): fase `''` (el modal de inicio recopila `{ sets }` con validaciones de unicidad/cobertura, shell-partida.js:735) y `TURNO_ACTIVO` (el panel legacy incluye la card **RESPUESTA**, información crítica del conductor — diseño 1b; el área la oculta con `mostrarRespuesta:false`) devuelven `[]` → legacy. Migrar en 8.5c extendiendo el contrato o moviendo la card al área del conductor. | Paso 8.5b.2 (detectado) | Media |
