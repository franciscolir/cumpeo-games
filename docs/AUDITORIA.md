...
| 131 | Pictionary no migrado a `accionesConductor` (M2-A): devuelve `[]` en todas las fases → fallback total al panel legacy. El bonus requiere payload compuesto `{ equipo, puntos }` (shell-partida.js:1166-1167). **Contrato extendido en 8.5c.1**: el descriptor `input` acepta `payload` y el bind envía `{ ...payload, valor }`, así que el bonus ya es expresable (`{ equipo }` en el payload estático + `puntos` digitado). Pendiente **8.5c.2**: migrar Pictionary (fases + bonus). | Paso 8.5b.2 (detectado) | Media |
| 132 | Rosco migrado parcialmente (M3-A): fase `''` (el modal de inicio recopila `{ sets }` con validaciones de unicidad/cobertura, shell-partida.js:735) y `TURNO_ACTIVO` (el panel legacy incluye la card **RESPUESTA**, información crítica del conductor — diseño 1b; el área la oculta con `mostrarRespuesta:false`) devuelven `[]` → legacy. **Contrato extendido en 8.5c.1**: descriptor `html` inyecta HTML sin escapar → la card RESPUESTA ya es expresable. Pendiente **8.5c.2**: migrar Rosco (TURNO_ACTIVO con `html` + botones; `''` depende del modal). | Paso 8.5b.2 (detectado) | Media |
...
| 129 | ~~Historia Enredada fase `VOTANDO` no puede usar el descriptor `input` del contrato: el binding del shell envía `{ valor: e.target.value }` (shell-partida.js:1471) pero el handler `asignar-puntos-historia` lee `payload.puntos || 0` (1241) → asignaría 0 puntos; un botón descriptor tampoco sirve (payload estático). Decisión operador 8.5b.1: `VOTANDO` devuelve `[]` → fallback legacy, D3 incumplida. Habilitar D3 exige modificar el shell.~~ **Cerrada en 8.5c.1**: el binding `input` ahora envía `{ ...payloadBase, valor }` con el `payload` opcional del descriptor → el contrato sí expresa payload compuesto. Nota: la migración de la fase `VOTANDO` de Historia (cambiar el handler para leer `valor`, o declarar `payload`) queda pendiente junto con Pictionary/Rosco. | Paso 8.5b.1 (detectado) / 8.5c.1 | ~~Media~~ Cerrada |
