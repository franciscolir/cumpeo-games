...
describe('seedJuegos', () => {
  ...
  it('registry con 7 defs crea 7 Juegos', async () => {
    const { services } = await bootstrap(adapter);
    const juegos = await services.juego.listarJuegos();
    expect(juegos.length).toBe(7);
    const codigos = juegos.map((j) => j.codigo).sort();
    expect(codigos).toEqual(['CANCION_INCOMPLETA', 'HISTORIA_ENREDADA', 'MEMORIA', 'PICTIONARY', 'QUE_PIENSA_EL_PUBLICO', 'ROSCO', 'TRIVIA']);
  });
