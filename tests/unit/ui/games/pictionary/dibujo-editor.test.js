...
const SET_ID = 'set-dibujo-1';
const PREFIX = 'pictionary-dibujo';
...
describe('renderEditorItemsPictionaryDibujo (re-export)', () => {
  let container;
  let app;
  ...
  it('es una función exportada', () => {
    expect(typeof renderEditorItemsPictionaryDibujo).toBe('function');
  });

  it('renderiza con mount e ids de pictionary-dibujo', async () => {
    await renderEditorItemsPictionaryDibujo(container, app, SET_ID);
  ...
  it('muestra la instrucción de dibujo', async () => {
    await renderEditorItemsPictionaryDibujo(container, app, SET_ID);
    expect(container.innerHTML).toContain('El representante dibuja en pizarra física.');
    expect(container.innerHTML).toContain('Conceptos del set — Dibujo');
  });

  it('inicializa estado con submodo DIBUJO en su idPrefix', async () => {
    await renderEditorItemsPictionaryDibujo(container, app, SET_ID);
    expect(container.__pictionaryEditorEstado[PREFIX].submodo).toBe('DIBUJO');
    expect(container.__pictionaryEditorEstado[PREFIX].items).toEqual([]);
  });

  it('agregar concepto persiste { concepto } sin prohibidas', async () => {
    await renderEditorItemsPictionaryDibujo(container, app, SET_ID);
    container.querySelector(`#item-concepto-${PREFIX}`).value = 'CASTILLO';
    await container.querySelector(`#btn-guardar-item-${PREFIX}`).click();

    expect(app.services.set.agregarItem).toHaveBeenCalledWith(SET_ID, { concepto: 'CASTILLO' });
