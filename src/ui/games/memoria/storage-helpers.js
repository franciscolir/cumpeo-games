/* =============================================================
   Storage helpers para el editor de Memoricé (7.2b).
   Subir / obtener URL / eliminar imágenes de items de memoria.
   ============================================================= */
 ...
 */
export async function subirImagenMemoria(app, file) {
  validarApp(app, 'subirImagenMemoria');

  if (!(file instanceof File)) {
    throw new Error('subirImagenMemoria: file debe ser un File');
  }
  if (typeof file.type !== 'string' || !file.type.startsWith('image/')) {
    throw new Error('subirImagenMemoria: file debe ser una imagen (image/*)');
  }
  if (file.size > MAX_IMAGEN_BYTES) {
    throw new Error('subirImagenMemoria: la imagen no puede superar 2MB');
  }

  const ext = MIME_TO_EXT[file.type] || 'bin';
  const path = `memoria/${crypto.randomUUID()}.${ext}`;
  const blob = file;
 ...
 */
export async function obtenerUrlImagenMemoria(app, storageRef) {
  validarApp(app, 'obtenerUrlImagenMemoria');
 ...
 * @throws {Error} si app/storage faltan.
 */
export async function eliminarImagenMemoria(app, storageRef) {
  validarApp(app, 'eliminarImagenMemoria');
